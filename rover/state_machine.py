...
class RoverBrain:
    ...
    def update(self, detections, frame_width, frame_height,
               boundary_side=None, heading=None, at_deposit=False):
        ...
        # --- AVOID: граница впереди (но НЕ во время DEPOSIT) ---
        ...
        if self.state == "SEARCH":
            ...
            return self._return(heading, at_deposit)
    ...
    def _return(self, heading, at_deposit=False):
        """RETURN: едем домой. Увидели жёлтое → DEPOSIT."""
        if at_deposit:
            self.state = "DEPOSIT"
            return "DEPOSIT"
